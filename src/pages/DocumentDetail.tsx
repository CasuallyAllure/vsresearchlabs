  ...
  return (
    <article className="py-[var(--space-8)] pb-[var(--space-24)] lg:pb-[var(--space-8)]">
      ...
      <nav className="mb-[var(--space-6)] text-xs uppercase tracking-widest text-white/40">
        <Link to="/documentation" className="hover:text-white/70 focus:outline-none focus-visible:ring-1 focus-visible:ring-white/30">
          Documentation
        </Link>
        ...
        <span className="mx-[var(--space-2)] text-white/20">/</span>
        <span className="text-white/60">{doc.documentType}</span>
      </nav>
      ...
      <div className="flex items-center flex-wrap gap-x-[var(--space-3)] gap-y-[var(--space-2)] mb-[var(--space-4)]">
        ...
        <BatchCode value={doc.batchId} className="text-white/35" />
        {doc.documentVersion && (
          <>
            <span className="text-white/15" aria-hidden="true">·</span>
      ...
      {/* Title block */}
      <h1 className="text-3xl sm:text-4xl font-light text-white tracking-tight leading-tight mb-[var(--space-2)]">
        {doc.productName}
      </h1>
      <p className="text-lg font-light text-white/45 tracking-tight mb-[var(--space-8)]">
        {doc.documentType}
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-x-[var(--space-10)] gap-y-[var(--space-8)]">
        ...
        <div className="lg:col-span-4">
          <div className="aspect-[3/4] w-full max-w-[280px] overflow-hidden bg-[#050505] border border-white/[0.09] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
            {doc.thumbnailUrl ? (
              <img
          ...
          {(doc.pageCount || doc.fileSizeKb) && (
            <div className="mt-[var(--space-3)] flex flex-wrap gap-x-[var(--space-4)] gap-y-[var(--space-1)]">
              {doc.pageCount !== undefined && (
                <span className="text-[10px] font-mono tabular-nums uppercase tracking-[0.15em] text-white/35">
        ...
        <div className="lg:col-span-8 flex flex-col gap-y-[var(--space-6)] lg:research-surface-solid lg:p-[var(--space-6)]">
            ...
            <dl className="border-t border-white/[0.06]">
              {doc.issuer && (
                <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                  <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Issuer</dt>
                  <dd className="text-sm text-white/70 text-right">{doc.issuer}</dd>
                </div>
              )}
              {doc.issuedBy && (
                <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                  <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Issued By</dt>
                  <dd className="text-sm text-white/70 text-right">{doc.issuedBy}</dd>
                </div>
              )}
              <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Issued</dt>
                <dd className="text-right">
                  <DateStamp iso={doc.issuedDate} className="text-white/70" />
                </dd>
              </div>
              {doc.reviewedAt && (
                <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                  <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Reviewed</dt>
                  <dd className="text-right">
                    <DateStamp iso={doc.reviewedAt} className="text-white/70" />
                  </dd>
                </div>
              )}
              {doc.expiresAt && (
                <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                  <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Expires</dt>
                  <dd
                    className={cn(
                      'text-sm font-mono tabular-nums text-right',
                      status === 'expired' ? 'text-red-400/70' : 'text-white/70',
                    )}
                  >
          ...
          <div>
            ...
            <dl className="border-t border-white/[0.06]">
              <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Batch ID</dt>
                <dd className="text-right">
                  <BatchCode value={doc.batchId} className="text-white/70" />
                </dd>
              </div>
              {doc.standardReference && (
                <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                  <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Standard</dt>
                  <dd className="text-sm font-mono tabular-nums text-white/70 text-right">{doc.standardReference}</dd>
                </div>
              )}
              {doc.documentVersion && (
                <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                  <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Revision</dt>
                  <dd className="text-sm text-white/70 text-right">{doc.documentVersion}</dd>
                </div>
              )}
              {doc.instrumentId && (
                <div className="flex items-baseline justify-between gap-[var(--space-4)] py-[var(--space-3)] border-b border-white/[0.06]">
                  <dt className="text-[11px] uppercase tracking-[0.2em] text-white/40 shrink-0">Instrument</dt>
                  <dd className="text-sm font-mono tabular-nums text-white/70 text-right">{doc.instrumentId}</dd>
                </div>
              )}
            </dl>
          </div>
      ...
      {associatedProduct && (
        ...
        >
          ...
          <Link
            ...
            className="group research-surface-solid p-[var(--space-4)] inline-flex items-center gap-[var(--space-4)] w-full sm:w-auto focus:outline-none focus-visible:ring-1 focus-visible:ring-white/25"
          >
            <AbbreviationChip value={associatedProduct.abbreviation} />
            <div className="flex-1 min-w-0">
              <p className="text-sm text-white group-hover:text-gold transition-colors truncate">
                {associatedProduct.name}
              </p>
              <SKUCode value={associatedProduct.sku} className="text-white/35" />
            </div>
            <span
              className="text-white/30 group-hover:text-gold group-hover:translate-x-0.5 transition-[color,transform] duration-150 shrink-0 ml-auto"
              aria-hidden="true"
            >
              →
            </span>
