...
// "Payment received — order processing" notification — the whole decision
// body, Deno-free.
//
...
export interface ProcessingNotificationConfig {
  supabaseUrl: string;
 ...
 *  factory). */
export interface ProcessingNotificationDeps {
  createClient: (url: string, key: string) => NotificationSupabaseClient;
  fetch: (input: string, init?: RequestInit) => Promise<Response>;
  requireAdmin: (req: Request) => Promise<AdminGateResult>;
}

export function createProcessingNotificationHandler(
  cfg: ProcessingNotificationConfig,
  deps: ProcessingNotificationDeps,
...
const handleProcessingNotification = async (req: Request): Promise<Response> => {
  ...
  return handleProcessingNotification;
}
