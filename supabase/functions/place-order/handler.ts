...
// Auto-invoice checkout — the WHOLE orchestration, Deno-free.
//
// Extracted verbatim from index.ts (2026-07-18) so vitest can drive every
// decision path of the checkout money engine directly. index.ts is now a thin
// Deno shim: it reads env, builds the real deps (supabase-js createClient,
// fetch, turnstile, telemetry, CORS), and mounts the handler this factory
...
export const TELEMETRY_FN = "place-order";
...
export type OrderQueryBuilder = any;

export interface OrderSupabaseClient {
 ...
 *  body has always used, so the body is byte-identical to the pre-extraction
...
export function createOrderHandler(
  ...
  const BRAND_STAMP_URL      = cfg.brandStampUrl;

  const CORS_HEADERS = cfg.corsHeaders;

  const { createClient, fetch, verifyTurnstile, clientIp, alertOperator, logEvent } = deps;
