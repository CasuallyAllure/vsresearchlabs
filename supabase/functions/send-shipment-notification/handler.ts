...
// "Your order has shipped" notification — the whole decision body, Deno-free.
//
...
export interface ShipmentNotificationConfig {
  supabaseUrl: string;
 ...
 *  factory). */
export interface ShipmentNotificationDeps {
  createClient: (url: string, key: string) => NotificationSupabaseClient;
  fetch: (input: string, init?: RequestInit) => Promise<Response>;
  requireAdmin: (req: Request) => Promise<AdminGateResult>;
}

export function createShipmentNotificationHandler(
  cfg: ShipmentNotificationConfig,
  deps: ShipmentNotificationDeps,
...
const handleShipmentNotification = async (req: Request): Promise<Response> => {
  ...
  return handleShipmentNotification;
}
