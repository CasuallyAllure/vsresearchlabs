...
// Branded PAID receipt — the WHOLE decision body, Deno-free.
//
// Extracted verbatim from index.ts (2026-07-18) so vitest can drive every
// decision path directly (tests/unit/sendReceiptHandler.test.ts). index.ts is
// now a thin Deno shim: it reads env, builds the real deps (supabase-js
// createClient, fetch, the _shared/adminGate.ts requireAdmin), and mounts the
...
// ../_shared/adminGate.ts — injected as deps.requireAdmin). Once gated,
...
export interface ReceiptHandlerConfig {
  supabaseUrl: string;
...
export type ReceiptQueryBuilder = any;

export interface ReceiptSupabaseClient {
...
export interface ReceiptHandlerDeps {
  createClient: (url: string, key: string) => ReceiptSupabaseClient;
  fetch: (input: string, init?: RequestInit) => Promise<Response>;
  requireAdmin: (req: Request) => Promise<AdminGateResult>;
}

export function createReceiptHandler(
  cfg: ReceiptHandlerConfig,
  deps: ReceiptHandlerDeps,
...
const handleReceipt = async (req: Request): Promise<Response> => {
