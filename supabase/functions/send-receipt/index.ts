...
// Deno shim for the branded PAID receipt.
//
// The WHOLE decision body lives in handler.ts (Deno-free, driven directly by
// the vitest suite — tests/unit/sendReceiptHandler.test.ts). This file is the
...
const handleReceipt = createReceiptHandler(
  {
  ...
  {
    // The handler only uses the structural slice ReceiptSupabaseClient names;
    // the cast erases supabase-js generics that don't survive injection.
    createClient: (url, key) => createClient(url, key) as unknown as ReceiptSupabaseClient,
    fetch: (input, init) => fetch(input, init),
    requireAdmin,
  },
);

Deno.serve(handleReceipt);
